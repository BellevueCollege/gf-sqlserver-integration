...
class DigitalMarketing_BAS {
	...
	public function save() {
		...
		if ( $conn ) {
			try {
				...
				$tsql   = 'EXEC [usp_InsertIntoBASDigitalMarketingForm]'
							. '@TransID = :TransID,'
							. '@Fname = :FirstName,'
							. '@Lname = :LastName,'
							. '@FormID = :FormID,'
							...
							. '@EmploymentStatus = :EmploymentLevel,'
				...
				error_log( print_r( 'PDOException in DigitalMarketing_BAS::save - ' . $e->getMessage(), true ) );
			} catch ( Exception $e ) {
				error_log( print_r( 'General exception in DigitalMarketing_BAS::save - ' . $e->getMessage(), true ) );
			}
		}
		return false;
	}
	...
	public function build( $_entry ) {
		...
		$this->emplid     = ! empty( $_entry['76'] ) ? rgar( $_entry, '76' ) : null;
		...
		} elseif ( ! empty( $_entry['67'] ) && strtolower( rgar( $_entry, '67' ) ) == 'yes' ) {
		...
		} elseif ( ! empty( $_entry['74'] ) && strtolower( rgar( $_entry, '74' ) ) == 'yes' ) {
